 ...
 */
async function uploadExpensesCsv(page, filePath) {
...
test.describe('Expenses CSV Upload and Filtering', () => {
    ...
    test('uploads CSV and filters update totals correctly @smoke', async ({ page }) => {
        ...
        await expect(page.getByTestId('expenses-error')).toHaveText('No expenses found for the selected filters. Try a different month or category.');
    });
